...
export type Unsigned<M extends BusMessage<T>, T extends string = string> = Omit<
  M,
  "signature"
>;
...
export interface BusConnector {
  listenTo: (
    topic: string,
    callback: <T extends string>(message: BusMessage<T>) => void
  ) => Promise<void>;
...
export const isBusMessage = <T extends string>(
  message: unknown
): message is BusMessage<T> => {
  return (
